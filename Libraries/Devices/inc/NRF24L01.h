...
#define NRF_CE_PIN          5   //C5
#define NRF_CSN_PIN         5   //A5
#define NRF_MOSI_PIN        23  //B23
#define NRF_SCK_PIN         20  //B20
                            
#define NRF_MISO_PIN        22  //B22
#define NRF_IRQ_PIN         21  //B21

#define NRF_MISO    (uint8)(((PTB->PDIR)>>NRF_MISO_PIN)&0x01)
#define NRF_CE(x)   ((x==0)?(PTC->PDOR&=~(uint32)(1<<NRF_CE_PIN)):(PTC->PDOR|=(uint32)(1<<NRF_CE_PIN)))
#define NRF_CSN(x)  ((x==0)?(PTA->PDOR&=~(uint32)(1<<NRF_CSN_PIN)):(PTA->PDOR|=(uint32)(1<<NRF_CSN_PIN)))
#define NRF_MOSI(x) ((x==0)?(PTB->PDOR&=~(uint32)(1<<NRF_MOSI_PIN)):(PTB->PDOR|=(uint32)(1<<NRF_MOSI_PIN)))
#define NRF_SCK(x)  ((x==0)?(PTB->PDOR&=~(uint32)(1<<NRF_SCK_PIN)):(PTB->PDOR|=(uint32)(1<<NRF_SCK_PIN)))
#define NRF_IRQ     (uint8)(((PTB->PDIR)>>NRF_IRQ_PIN)&0x01)
...
#define NRF_RF_CH       40                   // RF射频通道 0~125
...
extern uint8 NRF_RX_Buff[NRF_DATA_WIDTH];                      	// 接收数据缓冲区
extern uint8 NRF_TX_Buff[NRF_DATA_WIDTH]; 

#define CRC_C_SmartCar	0
